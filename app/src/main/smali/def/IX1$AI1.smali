.class Ldef/IX1$AI1;
.super Ldef/N50;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/EnclosingMethod;
    value = Ldef/IX1;-><init>(Landroidx/room/HRA;)V
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x0
    name = "AI1"
.end annotation


# instance fields
.field final synthetic d:Ldef/IX1;


# direct methods
.method constructor <init>(Ldef/IX1;Landroidx/room/HRA;)V
    .locals 0

    iput-object p1, p0, Ldef/IX1$AI1;->d:Ldef/IX1;

    invoke-direct {p0, p2}, Ldef/N50;-><init>(Landroidx/room/HRA;)V

    return-void
.end method


# virtual methods
.method public d()Ljava/lang/String;
    .locals 1

    const-string v0, "INSERT OR REPLACE INTO `SystemIdInfo` (`work_spec_id`,`system_id`) VALUES (?,?)"

    return-object v0
.end method

.method public bridge synthetic g(Ldef/TW1;Ljava/lang/Object;)V
    .locals 0

    check-cast p2, Ldef/GX1;

    invoke-virtual {p0, p1, p2}, Ldef/IX1$AI1;->i(Ldef/TW1;Ldef/GX1;)V

    return-void
.end method

.method public i(Ldef/TW1;Ldef/GX1;)V
    .locals 2

    iget-object v0, p2, Ldef/GX1;->a:Ljava/lang/String;

    const/4 v1, 0x1

    if-nez v0, :cond_0

    invoke-interface {p1, v1}, Ldef/RW1;->y(I)V

    goto :goto_0

    :cond_0
    invoke-interface {p1, v1, v0}, Ldef/RW1;->d(ILjava/lang/String;)V

    :goto_0
    iget p2, p2, Ldef/GX1;->b:I

    int-to-long v0, p2

    const/4 p2, 0x2

    invoke-interface {p1, p2, v0, v1}, Ldef/RW1;->k(IJ)V

    return-void
.end method
